include("/Users/fqiang/.julia/v0.4/ReverseDiffTape/interface/interface.jl")
using TapeInterface
...
function opf(bus, branch,
    ...
    mod = Model(solver=TapeSolver(IpoptSolver(max_iter=3)))
...
function report(mod,N)
    ...
    for i=1:N
        solve(mod)
        eval_f += mod.internalModel.evaluator.eval_f_timer
        eval_g += mod.internalModel.evaluator.eval_g_timer
        eval_grad_f += mod.internalModel.evaluator.eval_grad_f_timer
        eval_jac_g += mod.internalModel.evaluator.eval_jac_g_timer
        eval_hesslag += mod.internalModel.evaluator.eval_hesslag_timer
       
        jeval_f2 += mod.internalModel.evaluator.jeval_f_timer
        jeval_g2 += mod.internalModel.evaluator.jeval_g_timer
        jeval_grad_f2 += mod.internalModel.evaluator.jeval_grad_f_timer
        jeval_jac_g2 += mod.internalModel.evaluator.jeval_jac_g_timer
        jeval_hesslag2 += mod.internalModel.evaluator.jeval_hesslag_timer
    end
